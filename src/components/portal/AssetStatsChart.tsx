...
export const AssetStatsChart: React.FC<AssetStatsChartProps> = ({ assets }) => {
    ...
    function getColorForStatus(status: string) {
        switch (status) {
            case 'Published': return '#22c55e'; // green-500
            case 'In Review': return '#f97316'; // orange-500
            case 'Draft': return '#94a3b8'; // slate-400
            ...
            <div className="flex-1 flex flex-col items-center justify-center gap-6">
                ...
                <div className="relative w-48 h-48 flex-shrink-0">
                    <svg viewBox="-1.2 -1.2 2.4 2.4" className="w-full h-full">
                        ...
                        {stats.total === 0 && <circle cx="0" cy="0" r="1" fill="none" stroke="#e2e8f0" strokeWidth="0.4" className="dark:stroke-slate-700" />}
                        {slices}
                    </svg>
                    {/* Center Text */}
                    ...
                    {stats.data.map(item => (
                        <div key={item.name} className="flex items-center justify-between group">
                            <div className="flex items-center gap-2">
                                ...
                                <span className="text-sm text-slate-600 dark:text-slate-300 group-hover:text-slate-900 dark:group-hover:text-white transition-colors">{item.name}</span>
                            </div>
                            <div className="flex items-center gap-2">
