    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
        {assets.map((asset) => (
          <div
            key={asset.id}
            className="group relative bg-white dark:bg-stone-900 rounded-2xl overflow-hidden border border-slate-200 dark:border-stone-800 hover:border-amber-500/60 dark:hover:border-amber-700/60 transition-all duration-500 hover:shadow-2xl hover:shadow-amber-900/10"
          >
            {/* Image */}
            ...
            <div className="p-6">
              <div className="flex justify-between items-start mb-2">
                <h3 className="text-lg font-bold font-serif text-slate-900 dark:text-amber-50">
                  {asset.name}
                </h3>
              ...
              <p className="text-slate-500 dark:text-stone-400 text-sm mb-4 leading-relaxed line-clamp-2">
                {asset.type || '3D Model'} • {asset.size || 'Unknown size'} • Last updated{' '}
