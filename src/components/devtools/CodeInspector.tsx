    ...
    <>
      <div className="fixed bottom-4 left-4 z-[9999]" data-inspector-ui>
        <button
          ...
          className={`flex items-center gap-2 px-4 py-2 rounded-full font-mono text-xs font-bold shadow-lg transition-all border border-transparent ${
            enabled
              ? 'bg-indigo-600 text-white border-indigo-400'
              : 'bg-slate-800 text-slate-400 opacity-50 hover:opacity-100 hover:border-slate-600'
          }`}
        >
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex items-center justify-between gap-4 border-b border-gray-700 pb-2 mb-1">
                <div className="flex items-center gap-2 text-indigo-400 font-bold font-mono text-sm truncate">
                  {inspecting.tagName === 'img' ? (
                    <ImageIcon className="w-3 h-3" />
                  ) : (
                    <Box className="w-3 h-3" />
                  )}
                  <span className="uppercase">{inspecting.tagName}</span>
                  <span className="text-slate-500 font-normal text-xs ml-2 truncate max-w-[120px]">
                    in {inspecting.name}
                  </span>
                </div>
              ...
              <div className="grid grid-cols-2 gap-x-4 gap-y-1 text-[10px] text-slate-400 font-mono">
                ...
                <span className="text-slate-200">
                  {inspecting.width}px × {inspecting.height}px
                </span>
                {inspecting.src && (
                  <div className="col-span-2 truncate mt-1 pt-1 border-t border-slate-800">
                    ...
                    <span className="text-slate-300 truncate block" title={inspecting.src}>
                      {inspecting.src}
                    </span>
                  </div>
                )}
              ...
              {inspecting.file && (
                ...
                >
                  ...
                  <span
                    className={copied ? 'text-green-400' : 'text-slate-300 group-hover:text-white'}
                  >
              ...
              {!locked && (
                <div className="text-[9px] text-slate-600 text-center mt-1">
                  Press <span className="bg-slate-800 px-1 rounded text-slate-500">L</span> to lock
                  selection
                </div>
              )}
            </div>
