...
const Gallery: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAssets = async () => {
      try {
        setLoading(true);
        // In a real app, we'd filter by status='published'
        const data = await AssetsProvider.list();
        // Filter only published assets for the gallery
  ...
  return (
    <div className="bg-slate-50 dark:bg-slate-900 min-h-screen py-8 md:py-16">
        ...
        <div className="flex flex-col md:flex-row justify-between items-center gap-6 mb-12">
              ...
              <button
                ...
                className={`px-5 py-2 rounded-full text-sm font-bold transition-all whitespace-nowrap ${
                  filter === cat
                    ...
                    : 'bg-white dark:bg-slate-800 text-slate-600 dark:text-slate-300 border border-slate-200 dark:border-slate-700 hover:border-brand-300 dark:hover:border-brand-500 hover:bg-slate-50 dark:hover:bg-slate-700'
                }`}
              >
                {cat}
              </button>
          ...
          <div className="relative w-full md:w-80">
            <input
              ...
              className="w-full pl-10 pr-4 py-2 rounded-full border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-800 text-slate-900 dark:text-white focus:ring-2 focus:ring-brand-500 focus:border-transparent outline-none transition-all shadow-sm"
            />
          ...
          <div className="columns-1 sm:columns-2 lg:columns-3 xl:columns-4 gap-6 space-y-6">
            {filteredAssets.map((asset) => (
              <div
                ...
                className="group break-inside-avoid bg-white dark:bg-slate-800 rounded-2xl overflow-hidden border border-slate-200 dark:border-slate-700 hover:border-brand-300 dark:hover:border-brand-700 hover:shadow-2xl hover:shadow-brand-500/20 transition-all duration-300 cursor-pointer transform hover:-translate-y-2 hover:scale-[1.02]"
              >
                <div className="relative aspect-square overflow-hidden bg-slate-100 dark:bg-slate-700">
                  ...
                  <div className="absolute top-4 left-4 px-3 py-1.5 bg-white/95 dark:bg-slate-900/95 rounded-full text-xs font-bold text-slate-900 dark:text-white shadow-md backdrop-blur-sm border border-white/50 dark:border-slate-700/50">
                    {asset.type}
                  </div>
                </div>
                <div className="p-5">
      ...
      {selectedAsset && (
        ...
        >
          <div className="relative w-full max-w-6xl bg-white dark:bg-slate-900 rounded-3xl overflow-hidden shadow-2xl flex flex-col md:flex-row h-[85vh] md:h-[75vh] animate-slide-up">
            <button
            ...
            <div className="flex-1 bg-slate-100 dark:bg-slate-800 relative">
              ...
              >
                <div slot="progress-bar" className="absolute top-0 left-0 w-full h-1 bg-slate-200">
                  <div
            ...
            {/* Sidebar Details */}
            <div className="w-full md:w-96 bg-white dark:bg-slate-900 border-t md:border-t-0 md:border-l border-slate-200 dark:border-slate-800 p-6 md:p-8 flex flex-col overflow-y-auto">
              <div>
              ...
              <div className="space-y-3 mb-8 bg-slate-50 dark:bg-slate-800/50 rounded-xl p-4 border border-slate-100 dark:border-slate-800">
              ...
              <div className="mt-auto space-y-3">
                <button className="group w-full py-3.5 bg-brand-600 text-white rounded-xl font-bold hover:bg-brand-700 transition-all shadow-lg shadow-brand-500/30 hover:shadow-xl hover:shadow-brand-500/40 hover:scale-105 active:scale-100 flex items-center justify-center gap-2">
                  View in AR
                  <Box className="w-4 h-4 group-hover:rotate-12 transition-transform" />
                </button>
                <button className="group w-full py-3.5 border-2 border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 rounded-xl font-bold hover:bg-slate-50 dark:hover:bg-slate-800 hover:border-brand-300 dark:hover:border-brand-700 transition-all hover:scale-105 active:scale-100">
                  Share Model
                </button>
              </div>
            </div>
